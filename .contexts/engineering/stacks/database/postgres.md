...
- **Async I/O (AIO)**: subsystem que acelera sequential scans, bitmap heap scans e vacuum (`io_method`, `io_combine_limit`).
- **`uuidv7()` nativo**: UUIDs ordenáveis por tempo — alinhar com `@rules/data-modeling` e `@contracts/postgres` (preferir v7 sobre v4 para PKs).
- **Skip scan em B-tree multicoluna**: índices compostos úteis mesmo sem predicado na primeira coluna.
- **Generated columns virtuais** (default): calculadas na leitura; `STORED` quando precisar materializar.
- **RETURNING com OLD/NEW** em INSERT/UPDATE/DELETE/MERGE.
- **Temporal constraints** (`WITHOUT OVERLAPS` / `PERIOD`) para ranges em PK/UNIQUE/FK.
- **OAuth authentication** nativo no servidor (enterprise/IdP).
- **Checksums default no initdb**; MD5 password deprecated.
- Herdados de 16/17 e ainda relevantes: `pg_stat_io`, JSON constructors, logical replication em standby, parallel vacuum.
...
- `uuid-ossp` → preferir `uuidv7()` (PKs ordenáveis) ou `gen_random_uuid()` (v4) nativos no Postgres 18.
...
- Branded types para IDs (`type UserId = string & { readonly __brand: 'UserId' }`) — referencia `@stacks/language/typescript@7`.
...
- `pg_stat_statements`, `pg_stat_activity`, `pg_stat_io` (Postgres 16+, reforçado em 18).
