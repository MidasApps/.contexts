...
Resolva paths via `vite-tsconfig-paths` lendo o `tsconfig.json` do pacote. Não duplique aliases manualmente em `vitest.config.ts` — eles divergem e geram bugs sutis. Ver `@stacks/language/typescript@7`.
...
Import de locators no browser mode: `import { page } from 'vitest/browser'` (não de `@vitest/browser/context`).
...
Para o monorepo do projeto, use a opção `projects` no `vitest.config.ts` da raiz (substitui `vitest.workspace.ts` / `defineWorkspace` a partir do Vitest 3.2/4):
...
`pnpm vitest` na raiz executa todos os projetos com envs independentes. Cada pacote mantém seu `vitest.config.ts` com `environment`, `setupFiles` e thresholds próprios.
...
- Browser mode estável: visual regression via `toMatchScreenshot`, Playwright traces via `test.browser.trace`.
